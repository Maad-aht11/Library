...
def get_user_object(id_number):
    user_index = library.User.id_list.index(id_number)
    current_user = library.User.user_list[user_index]
    return current_user

#Create function to validate if user can check out books
def validate_check_out(id_number):
    current_user = get_user_object(id_number)
...
while (user_input != 5):
    ...
    if (user_input == '1'):
        ...
        if (id_input not in library.User.id_list):
            ...
            if (book_num_input.lower() == 'x'):
                ...
                if (is_allowed):
                    current_user = get_user_object(id_input)
                    current_user.books_checked_out.append(library.Book.available_books[book_num_input-1])
        ...
        library.User.id_list.append(user.id)
        library.User.user_list.append(user)
